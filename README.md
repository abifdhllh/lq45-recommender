...
| **Streamlit** | Dashboard: tema gelap default (`.streamlit/config.toml`) + toggle **Mode terang**; status snapshot, tanggal penutupan terakhir di data, peringatan unduhan parsial, filter kode, mode top-N / semua LQ45, tampilan ringkas untuk layar kecil |
...
- **Mode gelap / terang:** default **Mode gelap** memakai tema Streamlit (`config.toml`). **Matikan** untuk mode terang (override CSS); jika tabel terlihat aneh, pakai juga **⋮ → Settings → Theme**
